...
- **Toast 与语言切换**: `ToastProvider` 监听 `resolvedLocale` 变化，语言切换时自动清空所有已显示的 toast，避免旧语言文本残留。依赖 `t` 的 `useEffect`（如更新通知）会在语言变化后重新触发并用新语言弹出
- **Provider 嵌套顺序**: `ToastProvider` 必须位于 `I18nProvider` 内部，确保 toast 系统能感知语言变化。主窗口在 `App.tsx` 的 `AppWithProviders` 中保证此顺序；设置窗口在 `SettingsPage.tsx` 中自行嵌套 `I18nProvider > ToastProvider`
...
npm run pack:win-x64 # Windows NSIS 安装包 (仅 x64)
npm run pack:mac     # macOS DMG + ZIP
npm run lint         # ESLint 检查
npm run lint:fix     # ESLint 自动修复
```

本 fork 仅构建 Windows x64 和 macOS，Linux 构建和 Windows arm64 构建已从 CI 移除。
GitHub Actions workflow: `.github/workflows/release.yml`（仅 tag push `v*` 触发）。

### CI 打包脚本

CI 的 Windows 构建必须使用 `pack:win-x64`，**不能使用 `pack:win`**。上游的 `pack:win` 脚本会依次打 x64 和 arm64 两个包，第二次 arm64 打包时 electron-builder 会将 native modules（node-pty、serialport 等）重新编译为 arm64 架构，污染已打好的 x64 包或产出无法运行的 arm64 包。上游合并如果修改了 `package.json` 中的 `pack:win` 脚本定义，不影响 CI，但必须确认 `release.yml` 中引用的仍然是 `pack:win-x64`。
