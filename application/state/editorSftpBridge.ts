...
// `useSftpState` may be instantiated in multiple places (e.g. SftpView and
// ScpView), each owning its own pane registry. An editor tab opened from
// either path must be saved via the matching instance, so the bridge tracks
// all currently-mounted writers and dispatches by attempting each in turn
// until one succeeds.
//
